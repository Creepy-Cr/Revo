...
import { db, treasuryWalletTable, type TreasuryWallet } from "@workspace/db";
...
async function assertArcTestnet(): Promise<void> {
  ...
  try {
    ...
    throw new ChainError(
      "RPC_UNAVAILABLE",
      `Arc Testnet RPC is unreachable (${error instanceof Error ? error.message : String(error)}). No on-chain action was taken.`,
    );
  }
 ...
 */
export async function ensureTreasuryWallet(treasuryId: string): Promise<TreasuryWallet> {
  const [existing] = await db
    .select()
    .from(treasuryWalletTable)
    .where(eq(treasuryWalletTable.id, treasuryId));
  if (existing) return migratePlaintextKey(existing);
 ...
 */
async function migratePlaintextKey(wallet: TreasuryWallet): Promise<TreasuryWallet> {
...
function custodySigningKey(wallet: TreasuryWallet): Hex {
  if (wallet.encryptedKey) {
    ...
    if (unsealed.needsRewrap) {
      ...
      void db
        ...
        .then(() => logger.info("Custody envelope rewrapped with primary master secret"))
        .catch((err) => logger.error({ err }, "Custody envelope rewrap failed"));
    }
...
export async function verifyDeposit(
  ...
  try {
    ...
    throw new ChainError(
      "RPC_UNAVAILABLE",
      `Arc Testnet RPC failed while verifying: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
 ...
 * The API server is a single process, so an in-process mutex is sufficient;
 * the nonce itself is always re-read from the chain, never cached.
 */
const custodyQueues = new Map<string, Promise<unknown>>();
export function withCustodyLock<T>(treasuryId: string, fn: () => Promise<T>): Promise<T> {
...
export async function signUsdcTransfer(
  ...
  const account = privateKeyToAccount(custodySigningKey(wallet));
  ...
  try {
    ...
    throw new ChainError(
      "SEND_FAILED",
      `The withdrawal could not be prepared (nothing was broadcast): ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function confirmTransfer(hash: Hex): Promise<void> {
  ...
  try {
    ...
    throw new ChainError(
      "RPC_UNAVAILABLE",
      `The withdrawal was broadcast (tx ${hash}) but confirmation could not be observed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  if (receipt.status !== "success") {
