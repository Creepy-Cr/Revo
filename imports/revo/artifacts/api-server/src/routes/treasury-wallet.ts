...
export async function reconcilePendingWithdrawals(
  ...
  for (const row of rows) {
    ...
    if (row.txHash) {
      ...
      const outcome = await getTransferReceiptStatus(row.txHash as Hex);
...
router.post("/treasury/wallet/deposits", requireOperator(), async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (error instanceof ChainError) {
      req.log.warn({ txHash, code: error.code }, "Deposit claim rejected");
      res.status(chainErrorStatus(error)).json({ error: error.message });
      return;
    }
    throw error;
  }
...
router.post("/treasury/wallet/withdrawals", requireOperator(), async (req, res): Promise<void> => {
  ...
  try {
    await withCustodyLock(treasuryId, async () => {
      signed = await signUsdcTransfer(wallet, address, microUsdc);
      ...
      const [hashPersisted] = await db
        .update(onchainTransfersTable)
        .set({ txHash: signed.hash })
        .where(
          and(
            eq(onchainTransfersTable.treasuryId, treasuryId),
            eq(onchainTransfersTable.id, pending.id),
    ...
    if (!uncertain) {
      await refundAndFail(message);
      res.status(502).json({ error: `${message} The reserved units were returned to the treasury.` });
      return;
    }
    req.log.error(
      { transferId: pending.id, txHash: signed?.hash },
      "Broadcast outcome uncertain; left pending for reconciliation",
    );
    res.status(502).json({
      error: `${message} The withdrawal stays pending and will be reconciled automatically. Check the explorer: ${EXPLORER_URL}/tx/${signed?.hash}`,
    });
    return;
  }
  if (!signed) {
  ...
  try {
    ...
    if (error instanceof ChainError && error.code === "TX_REVERTED") {
      ...
      res.status(502).json({ error: `${error.message} The reserved units were returned to the treasury.` });
      return;
    }
    const message = error instanceof ChainError ? error.message : String(error);
    req.log.error({ transferId: pending.id, txHash }, "Withdrawal confirmation unobservable; left pending");
    res.status(502).json({
      error: `${message} The withdrawal stays pending. Check the explorer: ${EXPLORER_URL}/tx/${txHash}`,
    });
    return;
  }
