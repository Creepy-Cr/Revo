...
async function processTreasury(treasuryId: string, signal: AbortSignal): Promise<string | void> {
  ...
  try {
    await loadState(treasuryId, signal);
  } catch {
  ...
  if (!checkpoint?.lastScannedBlock) {
    ...
    await setCheckpoint(JOB_NAME, treasuryId, { lastScannedBlock: start.toString() }, signal);
  ...
  if (checkpoint.lastScannedHash) {
    ...
    if (anchor && anchor.hash.toLowerCase() !== checkpoint.lastScannedHash.toLowerCase()) {
      ...
      await setCheckpoint(
        JOB_NAME,
        treasuryId,
        { lastScannedBlock: rewound.toString() },
        signal,
      );
...
async function runDepositIndexer(signal: AbortSignal): Promise<string | void> {
