...
export async function processTreasury(
  ...
  if (!policy) {
    if (checkpoint.breachedPolicyId) {
      await setCheckpoint(JOB_NAME, treasuryId, { breachedPolicyId: null }, signal);
    }
    return;
  }
  ...
  if (breached && checkpoint.breachedPolicyId !== policy.id) {
    await setCheckpoint(JOB_NAME, treasuryId, { breachedPolicyId: policy.id }, signal);
...
async function runDrawdownMonitor(signal: AbortSignal): Promise<string | void> {
